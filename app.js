  ...
  return `
    <article class="business-card" tabindex="0" role="button" aria-label="Open ${escapeHTML(business.name)} profile" data-business-id="${business.id}" style="--card-bg:${business.color}">
      ...
      <div class="card-footer">
        <span><b>${business.universityShort}</b> · ${business.fulfillment}</span>
        <strong>View →</strong>
      </div>
    </article>`;
}
...
function handleSubmit(event) {
  ...
  document.querySelector("#reviewCard").innerHTML = `
    <div><h3>${escapeHTML(data.businessName)}</h3><p>${escapeHTML(window.UniBuddyStore.getUniversity(data.universityId).short)} · ${escapeHTML(data.category)} · ${escapeHTML(data.type)}</p></div>
...
submissionStatus.addEventListener("click", () => {
  ...
  document.querySelector("#reviewCard").innerHTML = `<div><h3>${escapeHTML(latest.businessName)}</h3><p>${escapeHTML(window.UniBuddyStore.getUniversity(latest.universityId).short)} · ${escapeHTML(latest.category)} · ${escapeHTML(latest.type)}</p></div><span class="status-pill pending">Under review</span>`;
  submissionFormView.hidden = true;
  document.querySelector(".submit-heading").hidden = true;
  submissionSuccess.hidden = false;
  submitDialog.showModal();

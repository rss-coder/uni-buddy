...
function renderQueue() {
  ...
  const items = allItems
    ...
    .filter(item => [item.businessName, item.category, item.type, item.description, store.getUniversity(item.universityId).name].join(" ").toLowerCase().includes(query))
...
function openReview(id) {
  ...
  document.querySelector("#reviewMetadata").textContent = `${store.getUniversity(item.universityId).name} · ${item.category} · Submitted ${formatDate(item.submittedAt)}`;
